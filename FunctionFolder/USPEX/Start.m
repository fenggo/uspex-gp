...
while 1                                            % 程序主循环
    ...
    if exist('Current_POP.mat')
       ...
       checkCompatibility();
       if isempty(strfind(ORG_STRUC.homePath, homePath)) || length(ORG_STRUC.homePath)~=length(homePath)
       ...
       load USPEX.mat
       if exist('FORCE.mat')                          % force data, for what
          load FORCE.mat
       end
       if exist('POOL.mat')                           % 数据池  
          load POOL.mat
       end
    ...
    ea_start = time();
    path(path,[USPEXPath '/FunctionFolder/USPEX/' calcTypeStr]);
    eval(['EA_' calcTypeStr '()']);              %%%%%%% EA_310
    ea_end   = time();
    disp(['Time usage for EA:' num2str(ea_end-ea_start)]);
