...
gens_  = listdir(getcwd())
# gens  = ['{:s}/POSCAR.{:s}_opt'.format(gen,gen) 
#          for gen in gens if isdir(gen) and not gen.startswith('.') 
#                             and not gen.startswith('t')
#                             # and not gen.startswith('l')
#                             and gen not in ('gens',) ]
gens = []
for gen in gens_:
    if gen.endswith('.gen') or gen.endswith('.traj') or gen.startswith('POSCAR'):
       if gen.endswith('.svg'):
          continue
       s  = gen.split('.')[0]
       if s == 'POSCAR':
          s = gen.split('.')[-1]
       gens.append('{:s}/POSCAR.{:s}_opt'.format(s,s))
...
for i,gen in enumerate(gens):
    ...
    if label in I:
       ...
       D[label]   = [density]
    # eb = e-ehb
    print('*{:6s}* NM: {:2d} ehbond: {:8.4f}, evdw: {:8.4f}, ebind: {:8.4f},'
...
for label in labels:
    ...
    for i,lb in enumerate(I[label]):
        ...
        # print(lb)
        if lb in left:
           plt.text(ehb[i]-0.005,d[i]+0.005,lb_,ha='center',color='k',
                 fontsize=5.5)
        elif lb in right:
           plt.text(ehb[i]+0.005,d[i]+0.005,lb_,ha='center',color='k',
                 fontsize=5.5)
        else:
           plt.text(ehb[i],d[i]+0.005,lb_,ha='center',color='k',
                 fontsize=5.5)

ax.set_ylabel(r'$Density$ ($g/cm^3$)', fontsize=12)
ax.set_xlabel(r'$- Intramolecular$ $HB$ $Energy$ ($eV$)', fontsize=12)
