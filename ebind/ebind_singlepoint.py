...
show   = ['d3','k1350','k1705','l1671','g1942','g1337','i937',
          't1504','t1505','a960','15',
...
for stru in ids:
    if stru.endswith('.gen') or stru.endswith('.traj') or stru.startswith('POSCAR'):
        ...
        for m in m_:
            ...
            #                  xcf=xcf,xca=xca,basistype='split')
            # atoms  = img[-1]
            atoms_ = single_point(atoms_,id=1,xcf=xcf,xca=xca,
                                 basistype='split',cpu=args.n)
           
            density_ = cleanup(m.label,atoms_) 

            emol  = atoms_.get_potential_energy()
